...
abstract contract Token is TokenInterface, Exponential, TokenErrorReporter {
    function initialize(ComptrollerInterface comptroller_,
                        uint initialExchangeRateMantissa_) public {
    ...
    function liquidateBorrowInternal(address borrower, uint repayAmount, TokenInterface tokenCollateral) internal nonReentrant returns (uint, uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
        ...
        error = tokenCollateral.accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted liquidation failed
        ...
        return liquidateBorrowFresh(msg.sender, borrower, repayAmount, tokenCollateral);
    }
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, TokenInterface tokenCollateral) internal returns (uint, uint) {
        /* Fail if liquidate not allowed */
        uint allowed = comptroller.liquidateBorrowAllowed(address(this), address(tokenCollateral), liquidator, borrower, repayAmount);
        if (allowed != 0) {
        ...
        /* Verify tokenCollateral market's block number equals current block number */
        if (tokenCollateral.accrualBlockNumber() != getBlockNumber()) {
        ...
        (uint amountSeizeError, uint seizeTokens) = comptroller.liquidateCalculateSeizeTokens(address(this), address(tokenCollateral), actualRepayAmount);
        require(amountSeizeError == uint(Error.NO_ERROR), "LIQUIDATE_COMPTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED");

        /* Revert if borrower collateral token balance < seizeTokens */
        require(tokenCollateral.balanceOf(borrower) >= seizeTokens, "LIQUIDATE_SEIZE_TOO_MUCH");

        // If this is also the collateral, run seizeInternal to avoid re-entrancy, otherwise make an external call
        uint seizeError;
        if (address(tokenCollateral) == address(this)) {
            seizeError = seizeInternal(address(this), liquidator, borrower, seizeTokens);
        } else {
            seizeError = tokenCollateral.seize(liquidator, borrower, seizeTokens);
        }
        ...
        emit LiquidateBorrow(liquidator, borrower, actualRepayAmount, address(tokenCollateral), seizeTokens);

        /* We call the defense hook */
        // unused function
        // comptroller.liquidateBorrowVerify(address(this), address(tokenCollateral), liquidator, borrower, actualRepayAmount, seizeTokens);
